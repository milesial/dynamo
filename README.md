...
- **Zero-config deploy ([DGDR](docs/kubernetes/api-reference.md#dynamographdeploymentrequest))** *(beta):* Specify model, HW, and SLA in one YAML — AIConfigurator auto-profiles the workload, Planner optimizes the topology, and Dynamo deploys
...
Also available: [`tensorrtllm-runtime:1.1.1`](https://docs.nvidia.com/dynamo/resources/release-artifacts) and [`vllm-runtime:1.1.1`](https://docs.nvidia.com/dynamo/resources/release-artifacts).
...
For production multi-node clusters, install the [Dynamo Platform](docs/kubernetes/installation-guide.md) and deploy with a single manifest:
